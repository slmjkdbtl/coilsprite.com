...
export function loop(action: (dt: number, t: number) => void) {
	let paused = false
...
function mapValues<K extends string | number | symbol, V>(
	obj: Record<K, V>,
	map: (v: V, k: K) => V,
) {
	const obj2 = {} as Record<K, V>
	for (const k in obj) {
		obj2[k] = map(obj[k], k)
	}
	return obj2
...
export type TweenVal = number | number[]

function lerp<V extends TweenVal>(a: V, b: V, t: number): V {
	if (Array.isArray(a) && Array.isArray(b)) {
		return a.map((v, i) => lerp(v, b[i], t)) as V
	} else {
		// @ts-ignore
		return a + (b - a) * t
	}
...
export default function tween<V extends TweenVal>(
	from: V,
	to: V,
	duration: number,
	setValue: (n: V) => void,
